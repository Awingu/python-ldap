...
class TestLDAPUrl(unittest.TestCase):
    ...
    def test_combo(self):
        u = MyLDAPUrl("ldap://127.0.0.1:1234/dc=example,dc=com"
            + "?attr1,attr2,attr3"
            + "?sub"
            + "?" + six.moves.urllib.parse.quote("(objectClass=*)")
            + "?bindname=" + six.moves.urllib.parse.quote("cn=d,c=au")
            + ",X-BINDPW=" + six.moves.urllib.parse.quote("???")
            + ",trace=8"
        )
    ...
    def test_parse_dn(self):
        ...
        expected_value = six.b("dn=str\xc3\xb6der.com")
        if six.PY3:
            expected_value = expected_value.decode('utf-8')
        self.assertEqual(u.dn, expected_value)
    ...
    def test_parse_filter(self):
        ...
        expected_value = six.b("(sn=Str\xc3\xb6der)")
        if six.PY3:
            expected_value = expected_value.decode('utf-8')
        self.assertEqual(u.filterstr, expected_value)
    ...
    def test_bad_urls(self):
        # TODO: Looks like this test was already broken. Most of the
        # supposed bad URLs here were actually parsed just fine by LDAPUrl.
        # Need to check LDAPUrl to see if that object's URL parser is wrong
        # or these test cases are what are actually wrong.
